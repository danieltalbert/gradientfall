...
- [x] Town of Bootstrap: buildings, 6–8 NPCs (mixed personalities), dialogue UI *(built: 13 procedurally generated buildings around a square — mayor's hall, bell tower, the jettied Warm Start Inn, Branna's open forge with live coals and her notched beam, Elowen's ledger house, Nessa's cottage, six homes, and the Mill at the pond with a wheel that turns at the terrain's own water level; stepping-stone roads, market stalls, the east gate, Cedric's fold, and a fingerpost aimed at the region's real coordinates; windows and lanterns light at dusk. **All 13 approved NPCs placed** (the WORLDBOOK counts Bootstrap complete at 13, above this line's 6–8), each built from its ContentDB entry with the props its approved description names, working its trade and turning to watch Kern. Dialogue UI: proximity prompt → code-drawn box with typewriter text, greeting/idle/farewell drawn from shuffled bags, first-meeting flags in the existing save shape. **Verified in Godot 4.7.1 this session** — clean import, clean headless boot, and the whole conversation chain driven at runtime by a harness; screenshots captured under software GL, so composition is eyes-verified but the Forward+ look (SDFGI/TAA/fog) still wants a live pass)*
