...
- [x] Knowledge charge v1: quiz prompt in combat charges a special ability *(`KnowledgeQuiz` offers an approved question a few seconds into a fight — never while exploring, never with the meter already full; you answer 1–4 / d-pad while the monster keeps coming. Right → focus, paid out by the question's own difficulty; wrong or missed → the right answer and its explanation, no penalty (teaching game); taking a hit breaks concentration and dismisses the card, so the loop is "make space in the fight, then answer". `QuizBank` gates the draw to campaign progress (WORLDBOOK: D1–2 anywhere → D3 after 3 shrines → D4 after 6 → D5 at the endgame), leans on the region's topics, and never repeats a question until the pool runs dry. Code-drawn `QuizCard` in the combat HUD's palette. **First milestone verified in-engine from an autonomous run** — Godot 4.7.1 runs in this sandbox: clean import (0 errors, 0 warnings), clean boot, `res://tests/quiz_selftest.tscn` 27/27 PASS, and four rendered screenshots in `docs/progress/`. A human at the controls still owes it a feel pass — see DEVLOG.)*
