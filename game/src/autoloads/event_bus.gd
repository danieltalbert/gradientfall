...
# Dialogue & interaction (Town of Bootstrap)
## A villager is close enough to talk to, or "" / "" when none is. `prompt` is
## the ready-made label ("Talk to Mara Mallow") so the UI stays content-blind.
...
## First time Kern speaks to someone (remembered in GameState.flags).
