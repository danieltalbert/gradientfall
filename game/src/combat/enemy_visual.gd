...
extends Node3D
## Code-built, cel-shaded monster bodies — silhouette-first per GDD §10, no
## imported assets. One generator dresses every Datasedge foe by BEHAVIOR:
##   * swarm   → a Stray Glitchling: a knee-high wobble of mismatched pixel-cubes
##   * melee   → a heavier bruiser with horn spikes
##   * ranged  → a floating single-eyed "watcher" with a lens ring
##   * dummy   → a straw sparring post (no wobble; a safe hit-test target)
...
## strike — both animate the shared toon shader's albedo tint.
...
func _process(delta: float) -> void:
	...
	# Cubes jitter in place — the "mismatched pixels" read. Dummies stay still.
	if _behavior != "dummy":
...
func _build_watcher(base: Color, accent: Color, size: float, eye_glow: float) -> void:
	var s: float = size
	height = 1.1 * s
	# A floating orb — one big lens eye, ringed.
